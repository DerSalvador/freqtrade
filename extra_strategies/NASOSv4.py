...
entry_params = {'base_nb_candles_entry': 8, 'ewo_high': 2.403, 'ewo_high_2': -5.585, 'ewo_low': -14.378, 'lookback_candles': 3, 'low_offset': 0.984, 'low_offset_2': 0.942, 'profit_threshold': 1.008, 'rsi_entry': 72}
# Sell hyperspace params:
exit_params = {'base_nb_candles_exit': 16, 'high_offset': 1.084, 'high_offset_2': 1.401, 'pHSL': -0.15, 'pPF_1': 0.016, 'pPF_2': 0.024, 'pSL_1': 0.014, 'pSL_2': 0.022}
...
class NASOSv4(IStrategy):
    ...
    fast_ewo = 50
    slow_ewo = 200
    lookback_candles = IntParameter(1, 24, default=entry_params['lookback_candles'], space='entry', optimize=True)
    profit_threshold = DecimalParameter(1.0, 1.03, default=entry_params['profit_threshold'], space='entry', optimize=True)
    ...
    rsi_entry = IntParameter(50, 100, default=entry_params['rsi_entry'], space='entry', optimize=False)
    # trailing stoploss hyperopt parameters
    # hard stoploss profit
    pHSL = DecimalParameter(-0.2, -0.04, default=-0.15, decimals=3, space='exit', optimize=False, load=True)
    # profit threshold 1, trigger point, SL_1 is used
    pPF_1 = DecimalParameter(0.008, 0.02, default=0.016, decimals=3, space='exit', optimize=False, load=True)
    pSL_1 = DecimalParameter(0.008, 0.02, default=0.014, decimals=3, space='exit', optimize=False, load=True)
    # profit threshold 2, SL_2 is used
    pPF_2 = DecimalParameter(0.04, 0.1, default=0.024, decimals=3, space='exit', optimize=False, load=True)
    pSL_2 = DecimalParameter(0.02, 0.07, default=0.022, decimals=3, space='exit', optimize=False, load=True)
    # Trailing stop:
    trailing_stop = True
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dont_entry_conditions.append(dataframe['close_1h'].rolling(self.lookback_candles.value).max() < dataframe['close'] * self.profit_threshold.value)
